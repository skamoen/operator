...
public class ProcessenLevel1CompleteLayer extends CCLayer {
	CCInstructionLayer overlay = new CCInstructionLayer();
	private final CCColorLayer gameLayer;

	public static CCScene scene() {
		CCScene scene = CCScene.node();
		CCLayer layer = new ProcessenLevel1CompleteLayer();
	...
	protected ProcessenLevel1CompleteLayer() {
		gameLayer = CCColorLayer.node(ccColor4B.ccc4(255, 255, 255, 255));

		overlay.setUpWithTitle("Super!", "", "Volgend spel", "ProcessenLevel1Complete");
		addChild(gameLayer);
		showOverlay();
		// Animation
